...
def after_comment_creation(sender,instance,created,*args,**kwargs):
    if created:
        text=str( instance.owner.username+" commented on your post: "+instance.content[0:30])
...
def after_friend_request(sender,instance,created,*args,**kwargs):
    if created:
        ...
        notify_instance = Notification.objects.create(sender=instance.sender, reciever=instance.receiver,text=text, notifyType="profileView",instance_id=instance.sender.id)
        notify_instance.save()
    else:
        print("Updating..")
...
def after_like_creation(sender,instance,created,*args,**kwargs):
    if created:
        text=str( instance.user.username+" liked your post: "+instance.post.content[0:30])
        if instance.post.owner != instance.user:
            notify_instance = Notification.objects.create(sender=instance.user, reciever=instance.post.owner,text=text, notifyType="postView_like",instance_id=instance.post.id)
            notify_instance.save()
    else:
        print("Updating..")
